...
def f(for_seed, n, upper_bound):
    ...
    square_list = []
    for i in square:
        square_list.extend(i)
    square_list = sorted(square_list)

    for i in square_list:
        if square_list.count(i) > 1:
            duplicates.add(i)

    if not duplicates:
        ordered_square = np.array(square_list)
        ordered_square = ordered_square.reshape(n,n)
        ordered_square = ordered_square.T
